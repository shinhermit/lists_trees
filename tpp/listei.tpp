...
template <typename Element>
Box<Element> * ListeIterative<Element>::_getBox(int pos)const{
  int i;
  Box<Element> * current;
...
ListeIterative<Element> &  ListeIterative<Element>::insert(int i, Element elt){
  ...
  if(0 <= i && i <= _size){
	...
	if(i < _size){ // insertion en milieu de liste
	  current = _getBox(i);
...
template <typename Element>
Element & ListeIterative<Element>::operator[](int i)const{
  if(_size == 0)
    throw ListeIterative<Element>::ListLogicFault("Logic error: attempt to read empty list");
  else{
    if(0 <= i && i < _size){
